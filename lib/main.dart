...
class _MyHomePageState extends State<MyHomePage> {
  ...
  Future<void> _incrementCounter() async {
    final remote = RestaurantRemoteDatasourceImpl(
      httpClient: HttpClientImpl(
        dio: Dio(),
      ),
    );

    final local = RestaurantLocalDatasourceImpl(
      localDatabase: LocalDatabaseImpl(
        database: await DatabaseHelper.startDatabase(),
      ),
    );

    final repository = RestaurantRepositoryImpl(
      restaurantRemoteDatasource: remote,
      restaurantLocalDatasource: local,
    );

    final response = await repository.getAllRestaurants();
